...
The functions in dfeshiny will store the users preference in a
cookie (`dfe_analytics`), which expires after 365 days. The user can
...
### All together basic implementation

Here is an example full app.R file that shows all components of a basic working installation with vanilla Shiny tabs for navigation.

``` {r eval=FALSE}
```

## Joining up with your UI structure

The default ID for the UI panels is `navlistPanel`, and this is the default setting in the _server functions, if this ID is different, you will need to set cookies_nav_id in `cookies_banner_server()` to match your ID in the UI.

### bslib using footer navigation

Our recommendation approach is to us [bslib](https://rstudio.github.io/bslib/) for the overall styling of your app, and [shinyGovstyle](https://dfe-analytical-services.github.io/shinyGovstyle/) for the government style components.

Assuming this is the case, here is an example app.R file showing a basic shinyGovstyle layout for the full app, linking to a cookies page from the footer.
